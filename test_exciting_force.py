...
<input>
  ...
  <structure speciespath="./">
    ...
    <species speciesfile="C.xml" rmt="1.10">
...
def main():
    ...
    ff = open("forces.dat", "w")
    
    for i in range(100):

        t = 0.2 + 0.1 * i / 99
        
        print "step : ", i, ", coord : ", t

        coord2 = "%f %f %f" % (t, t, t)

        fout = open("input.xml", "w")
        fout.write(inpxml.replace("@coord2",coord2))
        fout.close()
    
        new_env = copy.deepcopy(os.environ)
        new_env["OMP_NUM_THREADS"] = str(4)
        new_env["MKL_NUM_THREADS"] = str(4)

        fstdout = open("stdoutput.txt", "w");
        p = subprocess.Popen(["srun", "-n", str(4), "./excitingmpi"], cwd="./", env=new_env, stdout=fstdout)
        p.wait()
        fstdout.close()

        fin = open("force.txt", "r")
        ff.write(fin.read())
        fin.close()
